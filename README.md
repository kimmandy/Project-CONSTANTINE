...
| 이름 | 역할 | 소감 |
|------|------|------|
| 성화준 | 조장, 일정 관리, 티칭 | 유쾌한 분위기로 팀워크 중심을 잡았음 |
| 김경운 | PLC 래더 설계 | 실전 적용을 통해 자신감을 얻음 |
| 김명지 | 티칭, 디버깅 | 반복 테스트의 중요성을 배움 |
| 유현우 | EPLAN 설계, 센서 회로 | 하드웨어와 통신 이해도를 높임 |
| 정선우 | 배선, PLC 래더 설계 | 사용자 관점에서 시스템을 바라봄 |
| 정진효 | HMI 및 전장 구성 | 트러블슈팅 경험이 큰 자산이 됨 |
